...
// Last update Thu Jun 15 16:47:08 2017 maxence.lauque
//
...
{
  IAnimatedMesh          *mesh = smgr->getMesh("images/cube.obj");
  IAnimatedMeshSceneNode *node  = smgr->addAnimatedMeshSceneNode(mesh, 0, -1,
								 vector3df(0, 0, 0),
								 vector3df(0, 0, 0),
								 vector3df(25.5f, 26, 0.2f));
  if (node)
    {
      node->setMaterialFlag(EMF_LIGHTING, false);
      node->setMD2Animation(scene::EMAT_STAND);
      node->setMaterialTexture( 0, driver->getTexture("images/grass.jpg") );
      node->setPosition(vector3df(8.5f,-10,20));
    }
}
...
void					Map::fillMap()
{
  for (size_t i = 0; i <= _Map.size() - 1; ++i)
	...
	{
	  _Map[i][j]->SetAff();
	  if (_Map[i][j]->type == 0 && _Map[i][j]->_cont != NULL){
	    _Map[i][j]->_cont->SetAff();
	  }
	}
    }
}
